...
int main (int argc, char *argv[])
{
    ...
    char       net_buffi[SIZE];          // to hold the bytes read from socket
    ...
    inet_pton(AF_INET, argv[1], &serv_addr.sin_addr);//convert IP address from text to binary
    
    source_file = fopen(argv[3], "rb");

    connect(sockfd,(struct sockaddr *)&serv_addr, sizeof(serv_addr));//connect to server
    strncpy(file_buff, argv[4], strlen(argv[4]));//fill buff
    write(sockfd, file_buff, strlen(argv[4]));//write file name of destination to socket

    while(fread(net_buff, 10, 1, source_file)) {//copy buff
