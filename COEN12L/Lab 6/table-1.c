...
//Quick Sort
//Recursively sorts unsorted array using quick sort method
//O(nlogn)
...
void quickSort( SET *sp, void **data, int lo, int hi){
	if(lo < hi){
		...
		while(left <= right){	//finds values at center pivot and swaps them
			while((*sp->compare)(data[left],pivot) < 0){
				left++;
			}
			while((*sp->compare)(data[right],pivot) > 0){
				right--;
			}
			if(left <= right){
				temp = data[left];
		...
		quickSort(sp, data, lo, right);	//recursively sorts left side of array
		quickSort(sp, data, left, hi);	//recursively sorts right side of array
	}
}

//Get Elements
//Allocates and returns an array of all elements in the set
//O(n)
