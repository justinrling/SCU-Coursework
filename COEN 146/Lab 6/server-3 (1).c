...
int my_receive (PACKET *recv_pck)
{
	...
	while (1)
	{
		...
		printf("Receiving...\n");
		memset(recv_pck, '\0', sizeof(recv_pck));
		...
		printf("\n\n\n");
		printf("\nState: %d\n", recv_pck->header.seq_ack);
		printf("\nLength: %d\n", recv_pck->header.len);
		printf("\nData: %s\n", recv_pck->data);
		printf("\nChecksum: %d\n", recv_pck->header.checksum);
		printf("\nData Length: %ld", strlen(recv_pck->data));
		printf("\n\n\n");
		...
		else
		{
            ...
            ack_packet.checksum = 0;
			ack_packet.seq_ack = !state;
			//packet received last was the previous state
			...
			if(r>8){
				...
				if(r <=8)
					...
					printf("Error! Packet has been corrupted!\n");
